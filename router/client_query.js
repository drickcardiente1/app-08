...
client_queries.post('/reg_client', async (req, res) => {
    ...
    qry = `INSERT INTO clients(id, firstname, lastname, gender, contact, email, password, address, avatar, date_created) VALUES ('', '${req.body.firstname}', '${req.body.lastname}', '${req.body.gender}', '${req.body.contact}', '${req.body.username}', '${pwd}', '${req.body.address}', '${proccess.env.default_logo}', '${today}')`;
    (async () => {
        await new Promise((resolve, reject) => {
            db.query(qry, (err, data) => {
                if (err) {
